angular.module('lmn.samba_share').service('samba_share', function($rootScope, $http, $q) {
    ...
    this.chmod = (path, mode) =>
      $http.post(`/api/lmn/samba_share/chmod/${path}`, {mode}).then(response => response.data)

    this.createFile = (path) =>
      $http.post(`/api/lmn/samba_share/create-file/${path}`)

    this.createDirectory = (path) =>
      $http.post(`/api/lmn/samba_share/create-directory/${path}`)
