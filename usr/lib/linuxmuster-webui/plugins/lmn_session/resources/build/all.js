...
(function() {
  ...
  //        $scope.files = resp['data'][0]
  //        $scope.filesList = resp['data'][1]
  angular.module('lmn.session').controller('LMNSessionController', function($scope, $http, $location, $route, $uibModal, gettext, notify, messagebox, pageTitle, lmFileEditor, lmEncodingMap, filesystem, validation, $rootScope, wait, userPassword, lmnSession) {
    ...
    lmnSession.getParticipants(lmnSession.current.ID).then(function(resp) {
      ...
      return console.table($scope.participants);
    });
    $scope.checkboxModel = {
      value1: false,
      value2: true
    ...
    return $scope.notImplemented = function(user) {
      return messagebox.show({
