...
angular.module('lmn.setup_wizard').controller('InitAccountController', function ($scope, $location, $http, gettext, pageTitle, notify) {
    ...
    this.validPW = () => {
        if (this.ini.adminpw) {
            no_hash = this.ini.adminpw.indexOf('#') < 0;
        } else {
            no_hash = false;
        }
        return validCharPwd(this.ini.adminpw) && no_hash
    }
...
angular.module('lmn.setup_wizard').controller('InitDoneController', function ($window, $http, gettext, pageTitle, core, notify, $timeout, messagebox, $q) {
