...
class API(object):
    ...
    def list_settings(self, filters: Dict[str, Union[str, Pattern]]=None, order_by: str=None) -> list:
        """
        List all available settings from the api
        ...
        r = self._session.get("{}/api/s/{}/get/setting".format(self._baseurl, self._site, verify=self._verify_ssl), data="json={}")
    ...
    def list_devices(self, filters: Dict[str, Union[str, Pattern]]=None, order_by: str=None) -> list:
        """
    ...
    def modify_device(self, deviceId: str, modifyData) -> list:
        """
        Modify data of a device

        :param deviceId: Id of of the device
        :param modifyData: dict with valid key, value pairs
        :return: A list of the modified devices on the format of a dict
        """

        r = self._session.put("{}/api/s/{}/rest/device/{}".format(self._baseurl, self._site, deviceId), verify=self._verify_ssl, data=json.dumps(modifyData))
        self._current_status_code = r.status_code
    ...
    def get_tags(self) -> list:
        """
        Get all available tags and Macs of devices

        :return: A list of tags in form of a dict
        """

        r = self._session.get("{}/api/s/{}/rest/tag".format(self._baseurl, self._site), verify=self._verify_ssl)
        self._current_status_code = r.status_code
