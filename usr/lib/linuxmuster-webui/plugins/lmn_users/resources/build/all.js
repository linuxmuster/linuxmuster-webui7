...
(function() {
  angular.module('lmn.users').config(function($routeProvider) {
    return $routeProvider.when('/view/lmn/users/parents', {
  ...
  angular.module('lmn.users').controller('LMUsersParentsController', function($scope, $http, $location, $route, $uibModal, gettext, notify, messagebox, pageTitle, customFields, userPassword) {
    ...
    $scope.all_selected = false;
    $scope.query = '';
    customFields.load_display('parents').then(function(resp) {
      $scope.customDisplay = resp['customDisplay'];
      return $scope.customTitle = resp['customTitle'];
    });
    $scope.isListAttr = function(attr) {
    ...
    $http.get('/api/lmn/sophomorixUsers/parents').then(function(resp) {
      return $scope.parents = resp.data;
    });
    $scope.showFirstPassword = function(username) {
      $scope.blurred = true;
      return userPassword.showFirstPassword(username).then(function(resp) {
    ...
    $scope.setCustomPassword = userPassword.setCustomPassword;
    $scope.batchResetFirstPassword = function() {
      return userPassword.batchPasswords($scope.parents, 'reset-first');
    };
    $scope.batchSetRandomFirstPassword = function() {
      return userPassword.batchPasswords($scope.parents, 'random-first');
    };
    $scope.batchSetCustomFirstPassword = function() {
      return userPassword.batchPasswords($scope.parents, 'custom-first');
    };
    $scope.printSelectedPasswords = function() {
      return userPassword.printSelectedPasswords($scope.parents);
    };
    $scope.userInfo = function(user) {
      return $uibModal.open({
        templateUrl: '/lmn_users:resources/partial/userDetails.modal.html',
    ...
    return $scope.selectAll = function(query) {
      ...
      for (i = 0, len = ref.length; i < len; i++) {
        ...
        if (parent.sAMAccountName.toLowerCase().includes(query.toLowerCase())) {
          results.push(parent.selected = $scope.all_selected);
        } else {
          results.push(void 0);
        }
      }
...
(function() {
  ...
  angular.module('lmn.users').controller('LMUsersStaffController', function($scope, $http, $location, $route, $uibModal, gettext, notify, messagebox, pageTitle, customFields, userPassword) {
    ...
    $scope.all_selected = false;
    $scope.query = '';
    customFields.load_display('staff').then(function(resp) {
      $scope.customDisplay = resp['customDisplay'];
      return $scope.customTitle = resp['customTitle'];
    });
    $scope.isListAttr = function(attr) {
    ...
    $http.get('/api/lmn/sophomorixUsers/staff').then(function(resp) {
      return $scope.staff = resp.data;
    });
    $scope.showFirstPassword = function(username) {
      $scope.blurred = true;
      return userPassword.showFirstPassword(username).then(function(resp) {
    ...
    $scope.setCustomPassword = userPassword.setCustomPassword;
    $scope.batchResetFirstPassword = function() {
      return userPassword.batchPasswords($scope.staff, 'reset-first');
    };
    $scope.batchSetRandomFirstPassword = function() {
      return userPassword.batchPasswords($scope.staff, 'random-first');
    };
    $scope.batchSetCustomFirstPassword = function() {
      return userPassword.batchPasswords($scope.staff, 'custom-first');
    };
    $scope.printSelectedPasswords = function() {
      return userPassword.printSelectedPasswords($scope.staff);
    };
    $scope.userInfo = function(user) {
      return $uibModal.open({
        templateUrl: '/lmn_users:resources/partial/userDetails.modal.html',
    ...
    return $scope.selectAll = function(query) {
      ...
      for (i = 0, len = ref.length; i < len; i++) {
        ...
        if (st.sAMAccountName.toLowerCase().includes(query.toLowerCase())) {
          results.push(st.selected = $scope.all_selected);
        } else {
          results.push(void 0);
        }
      }
...
(function() {
  ...
  angular.module('lmn.users').controller('LMNUserDetailsController', function($scope, $route, $uibModal, $window, $uibModalInstance, $http, gettext, notify, messagebox, pageTitle, id, role, identity, customFields) {
      ...
      return messagebox.show({
        ...
        return $http.post('/api/lmn/sophomorixUsers/killuser', {
          ...
          if (resp.data === true) {
            ...
            return notify.error(`There was an error during the process. Please try manually to execute the command 'sophomorix-kill --kill ${user.cn}' to get more informations.`);
          }
        });
      });
    };
...
(function() {
  ...
  angular.module('lmn.users').controller('LMUsersListManagementController', function($scope, $http, $location, $route, $uibModal, gettext, hotkeys, notify, lmEncodingMap, messagebox, pageTitle, lmFileEditor, lmFileBackups, filesystem, validation) {
    ...
    $scope.parents_remove = function(parent, index) {
      if (parent._isNew) {
        $scope.cleanupEmptyRow(index, "parents");
      }
      return $scope.parents.remove(parent);
    };
    $scope.staff_remove = function(st, index) {
      if (st._isNew) {
        $scope.cleanupEmptyRow(index, "staff");
      }
      return $scope.staff.remove(st);
    };
    ...
    $scope.getparents = function(force = false) {
      if (!$scope.parents || force) {
        return $http.get("/api/lmn/users/lists/parents").then(function(resp) {
          return $scope.parents = resp.data;
        });
      }
    };
    $scope.getstaff = function(force = false) {
      if (!$scope.staff || force) {
        return $http.get("/api/lmn/users/lists/staff").then(function(resp) {
          return $scope.staff = resp.data;
        });
      }
    };
    ...
    $scope.parents_save = function() {
      ...
      $scope.show_errors = false;
      $scope.parents_first_save = false;
      return $http.post(`/api/lmn/users/lists/parents?encoding=${$scope.parents_encoding}`, $scope.parents).then(function() {
        return notify.success(gettext('Saved'));
      });
    };
    $scope.staff_save = function() {
      if ($scope.numErrors()) {
      ...
      $scope.show_errors = false;
      $scope.staff_first_save = false;
      return $http.post(`/api/lmn/users/lists/staff?encoding=${$scope.parents_encoding}`, $scope.staff).then(function() {
        return notify.success(gettext('Saved'));
      });
    };
